...
func buildClaudeFileSource(mediaMessage dto.MediaContent) types.FileSource {
	if mediaMessage.Type != dto.ContentTypeFile {
		return mediaMessage.ToFileSource()
	}
	...
	if file.FileName != "" {
		...
		if ext != "" {
			detectedMimeType := service.GetMimeTypeByExtension(ext)
...
func decodeClaudeTextContent(base64Data string) (string, error) {
		...
		if message.Role == "system" {
			...
			if message.Role == "tool" {
				...
				for _, mediaMessage := range message.ParseContent() {
					...
					default:
						source := buildClaudeFileSource(mediaMessage)
						if source == nil {
							continue
						}
						...
						if mediaMessage.Type == dto.ContentTypeFile && strings.HasPrefix(mimeType, "text/") {
							textContent, err := decodeClaudeTextContent(base64Data)
							if err != nil {
